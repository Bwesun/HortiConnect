  ...
  return (
    <IonPage>
      <IonContent className="">
        <h1 className='text-2xl font-extrabold text-gray-800 p-4'>Cluster Directory</h1>
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 p-4">
          {groups.map((group, index) => (
            <div key={index} className="bg-white rounded-lg shadow-md overflow-hidden">
              <IonImg src={group.image} alt={group.crop} className="w-full h-48 object-cover" />
              <div className="p-4">
                <h2 className="text-xl font-semibold text-gray-800">{group.name}</h2>
                <p className="text-gray-600 flex gap-3 items-center"><MapPinIcon size={18} /> {group.location}</p>
                <p className="text-gray-600 flex gap-3 items-center"><UsersIcon size={18} /> {group.members} members</p>
                <p className="text-gray-600 flex gap-3 items-center"><IonBadge  className='px-3 py-1 rounded-full bg-[#fdd8ad] text-amber-600 text-sm font-medium'>{group.crop}</IonBadge></p>
                <IonButton routerLink='/viewcluster/2' color="primary" expand="block" className="mt-4">View Details</IonButton>
              </div>
            </div>
          ))}
        </div>
